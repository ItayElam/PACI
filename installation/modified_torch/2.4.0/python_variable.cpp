...
// static int THPVariable_custom_set_grad_fn(
//     THPVariable* self,
//     PyObject* obj,
//     void* unused) {
//   HANDLE_TH_ERRORS
//   if (check_has_torch_function((PyObject*)self)) {
//     return handle_torch_function_setter(self, "grad_fn", obj);
//   }
...
//   if (obj == Py_None) {
//     // Detach the tensor if obj is None
//     THPVariable_Unpack(self).detach_();
//   } else {
//     // **New code begins here**

//    // Check if obj is an instance of torch.autograd.Function
//     TORCH_CHECK(
//         THPFunction_Check(obj),
//         "grad_fn must be a 'torch.autograd.Function' or its subclass");

//     // Convert PyObject* to THPFunction*
//     THPFunction* py_function = (THPFunction*)obj;

//     // Get the shared_ptr to the Node
//     std::shared_ptr<torch::autograd::Node> grad_fn = py_function->cdata.lock();

//     // Ensure grad_fn is valid
//     TORCH_CHECK(
//         grad_fn,
//         "The provided 'torch.autograd.Function' has expired or is invalid");

//     // Set the grad_fn of the variable
//     torch::autograd::impl::set_gradient_edge(
//         THPVariable_Unpack(self),
//         torch::autograd::Edge(grad_fn, 0));
//   }
//   return 0;
...
static int THPVariable_set_grad_fn(
  ...
  if (obj == Py_None) {
    ...
    // Check if obj is a Python Function or C++ FunctionBase
    bool is_py_function = THPFunction_Check(obj);
    bool is_cpp_function = THPCppFunction_Check(obj);

    TORCH_CHECK(
        is_py_function || is_cpp_function,
        "_grad_fn must be a 'torch.autograd.Function' or a 'torch._C._FunctionBase' instance");

    std::shared_ptr<torch::autograd::Node> grad_fn;

    if (is_py_function) {
      // If obj is a Python-defined autograd.Function
      THPFunction* py_function = (THPFunction*)obj;
      grad_fn = py_function->cdata.lock();
      TORCH_CHECK(
          grad_fn,
          "The provided 'torch.autograd.Function' has expired or is invalid");
    } else if (is_cpp_function) {
      // If obj is a C++ Function (torch._C._FunctionBase)
      THPCppFunction* cpp_function = (THPCppFunction*)obj;
      grad_fn = cpp_function->cdata;
      TORCH_CHECK(
          grad_fn,
          "The provided 'grad_fn' is invalid or does not have a valid C++ Node");
    } else {
      TORCH_CHECK(false, "_grad_fn must be a valid autograd Function");
    }
